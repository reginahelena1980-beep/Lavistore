...
import { Lock, ArrowLeft, KeyRound, Eye, EyeOff, Sparkles, ShieldCheck, Check, AlertCircle, RefreshCw } from 'lucide-react';
...
export const AdminLogin: React.FC<AdminLoginProps> = ({
  ...
  // Change password form state (Primeiro Acesso / Alteração)
  ...
  // Change Password Submit (Primeiro Acesso ou Alteração Voluntária)
  const handleChangePasswordSubmit = async (e: React.FormEvent) => {
    ...
    if (!current) {
      setChangeError('Por favor, digite a senha atual (padrão inicial: 1234).');
      return;
    }
    ...
    try {
      ...
      if (!serverSuccess && current !== savedPass && current !== '1234' && current !== 'admin') {
        setChangeError('A senha atual informada está incorreta.');
        setIsChangingPassword(false);
        return;
      }
        ...
        <div className="bg-white/95 backdrop-blur-md rounded-3xl border-2 border-purple-200 shadow-xl p-6 sm:p-8 space-y-6">
          ...
          <div className="text-center space-y-3">
            ...
            <h1 className="font-['Mali'] text-2xl font-bold text-purple-950">
              {mode === 'login' ? 'Acesso Administrativo' : 'Cadastrar Nova Senha'}
            </h1>
            <p className="text-xs text-slate-600 font-medium leading-relaxed">
              {mode === 'login' 
                ...
                : 'Defina uma senha pessoal e exclusiva para o painel administrativo da Lavistore.'}
            </p>
          </div>

          {/* Mode Switcher Tabs */}
          <div className="grid grid-cols-2 p-1 bg-purple-100/60 rounded-2xl border border-purple-200 text-xs font-bold">
            <button
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              <div className="pt-2 text-center">
                <button
                  ...
                  className="text-xs text-purple-800 hover:text-purple-950 font-bold hover:underline inline-flex items-center gap-1.5 cursor-pointer"
                >
                  <KeyRound className="w-3.5 h-3.5 text-amber-600" />
                  <span>Primeiro acesso? Clique para alterar a senha inicial</span>
                </button>
              </div>
          ...
          {/* VIEW 2: CHANGE PASSWORD / PRIMEIRO ACESSO */}
          {mode === 'change_password' && (
            <form onSubmit={handleChangePasswordSubmit} className="space-y-4">
              ...
              <div className="p-3 bg-amber-50/90 border border-amber-300 rounded-2xl text-xs text-amber-950 font-medium flex items-start gap-2.5">
                ...
                <div>
                  <p className="font-bold">Primeiro acesso ou alteração de segurança</p>
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-bold text-purple-950 flex items-center justify-between">
                  <span>Senha Atual / Provisória</span>
                  <span className="text-[11px] text-slate-400 font-normal">Padrão: 1234</span>
                </label>
                <div className="relative">
                  <input
